    ...
    internal class SqlServerSQLTranslator : StandardSQLTranslator
    {
        public override void Visit(Request request)
        {
            ...
            else if (request.RequestType == RequestType.Select)
            {
                ...
                {
                    sb.Append(_sqlElements.Pop());
                    if (request.Take > 0 || request.Skip > 0) sb.Append(" AS P_S_").Append(i);
                    sb.Append(", ");
                }
                sb.Length -= 2;
            ...
            if (request.Take > 0 || request.Skip > 0)
            {
                ...
                sb.Append(" ORDER BY T_PAGINATED.DSRANK");

                for (var i = 0; i < request.Orders.Count; i++) _sqlElements.Pop();
            }
            else
            {
            ...
            if (request.RequestType == RequestType.InsertValues) sb.Append("; SELECT SCOPE_IDENTITY()");
